...
export default function Dashboard() {
  ...
  const [detailsModal, setDetailsModal] = useState({ open: false, title: '', contentRaw: '', summary: null, showRaw: false });
  ...
  // Anti-spam states
  const [antiSpamStats, setAntiSpamStats] = useState(null);
  ...
  const [loadingAntiSpam, setLoadingAntiSpam] = useState(false);
  const [busyModeConfig, setBusyModeConfig] = useState({
  ...
  // Fetch anti-spam data when tab is activated
  useEffect(() => {
    if (activeTab === 'anti-spam' && token) {
      fetchAntiSpamData();
    }
  }, [activeTab, token]);
  ...
  const fetchAntiSpamData = async () => {
    try {
      setLoadingAntiSpam(true);

      // Fetch all anti-spam data in parallel
      const [statsRes, busyModeRes, blockedIPsRes, blockedDevicesRes, spamLogsRes] = await Promise.all([
        api.get('/anti-spam/stats'),
        api.get(`/anti-spam/busy-mode-status/${store?.id}`),
        api.get('/anti-spam/blocked-ips'),
        api.get('/anti-spam/blocked-devices'),
        api.get('/anti-spam/spam-logs?page=1&limit=20')
      ]);

      if (statsRes.data.success) {
        setAntiSpamStats(statsRes.data.data);
      }

      if (busyModeRes.data.success) {
        setBusyModeStatus(busyModeRes.data.data);
        setBusyModeConfig({
          maxOrdersPerWindow: busyModeRes.data.data.maxOrdersPerWindow || 20,
          timeWindowMinutes: busyModeRes.data.data.timeWindowMinutes || 15
        });
      }

      if (blockedIPsRes.data.success) {
        setBlockedIPs(blockedIPsRes.data.data || []);
      }

      if (blockedDevicesRes.data.success) {
        setBlockedDevices(blockedDevicesRes.data.data || []);
      }

      if (spamLogsRes.data.success) {
        setSpamLogs(spamLogsRes.data.data.logs || []);
      }
    } catch (error) {
      console.error('Fetch anti-spam data error:', error);
      toast.error('Không thể tải dữ liệu chống spam');
    } finally {
      setLoadingAntiSpam(false);
    }
  };

  const handleBlockDevice = async (deviceId) => {
    if (!deviceId) return;
    const confirm = window.confirm(`Bạn có chắc chắn muốn chặn thiết bị ${deviceId.substring(0, 12)}...?`);
    if (!confirm) return;

    try {
      setLoadingAntiSpam(true);
      const res = await api.post('/anti-spam/block-device', {
        deviceId,
      ...
      if (res.data && res.data.success) {
        toast.success(res.data.message || 'Đã chặn thiết bị');
        // Refresh data
        await fetchAntiSpamData();
      } else {
        toast.error(res.data.message || 'Chặn thiết bị thất bại');
      }
    } catch (error) {
      console.error('Block device error:', error);
      toast.error(error.response?.data?.message || 'Lỗi khi chặn thiết bị');
    } finally {
      setLoadingAntiSpam(false);
    }
  };
  ...
  const handleUnblockDevice = async (deviceId) => {
    if (!deviceId) return;
    const confirmUnblock = window.confirm(`Bạn có chắc chắn muốn huỷ chặn thiết bị ${deviceId.substring(0, 12)}...?`);
    if (!confirmUnblock) return;

    try {
      setLoadingAntiSpam(true);
      const res = await api.post('/anti-spam/unblock-device', {
        deviceId
      });

      if (res.data && res.data.success) {
        toast.success(res.data.message || 'Đã huỷ chặn thiết bị');
        await fetchAntiSpamData();
      } else {
      ...
      toast.error(error.response?.data?.message || 'Lỗi khi huỷ chặn thiết bị');
    } finally {
      setLoadingAntiSpam(false);
    }
  };
  ...
  const renderLogDetails = (log) => {
    ...
    try {
      ...
      // not JSON
      return (
        <span title={String(log.details)}>
          {String(log.details).substring(0, 120)}{String(log.details).length > 120 ? '...' : ''}
        </span>
      );
    }

    // device_spam_attempt
    if (log.action && log.action.includes('device_spam_attempt')) {
      return (
        <div className="text-sm space-y-1">
          {parsed.existingOrderId && <div>Đang có đơn đang xử lý: <span className="font-mono">{parsed.existingOrderCode || parsed.existingOrderId}</span></div>}
          {parsed.timeSinceLastOrder && <div>Thời gian kể từ đơn trước: <span className="font-mono">{Math.round(parsed.timeSinceLastOrder/1000)} giây</span></div>}
          {parsed.timeWindowMs && <div>Khoảng thời gian giám sát: <span className="font-mono">{Math.round(parsed.timeWindowMs/60000)} phút</span></div>}
        </div>
      );
    }

    // device_blocked_attempt
    if (log.action && log.action.includes('device_blocked_attempt')) {
      return (
        <div className="text-sm space-y-1">
          {parsed.reason && <div>Lý do: <span className="font-mono">{parsed.reason}</span></div>}
          <div>Loại block: <span className="font-mono">{parsed.isPermanent ? 'Vĩnh viễn' : `${parsed.remainingMinutes || '—'} phút`}</span></div>
        </div>
      );
    }

    // rate limit blocked / exceeded
    if (log.action && log.action.includes('rate_limit')) {
      return (
        <div className="text-sm space-y-1">
          {parsed.remainingBlockTime !== undefined && <div>Thời gian chặn còn lại: <span className="font-mono">{parsed.remainingBlockTime} phút</span></div>}
          {parsed.ordersInWindow !== undefined && <div>Số đơn trong khoảng: <span className="font-mono">{parsed.ordersInWindow}</span></div>}
          {parsed.maxOrders !== undefined && <div>Giới hạn: <span className="font-mono">{parsed.maxOrders}</span></div>}
        </div>
      );
    }

    // busy mode manual enable/disable
    if (log.action && log.action.includes('busy_mode_manual')) {
      const admin = parsed.adminUserId ? `Người quản trị #${parsed.adminUserId}` : 'Người quản trị';
      const status = parsed.enabled ? 'bật' : 'tắt';
      return (
        <div className="text-sm space-y-1">
          <div>{admin} đã <span className="font-semibold">{status}</span> chế độ "Quán đang bận"</div>
          <button
            className="text-xs text-blue-600 underline mt-1"
            onClick={() => setDetailsModal({ open: true, title: 'Chi tiết log', contentRaw: JSON.stringify(parsed, null, 2), summary: renderLogDetails(log), showRaw: false })}
          >
            Xem chi tiết
          </button>
        </div>
    ...
    // busy mode config updated
    if (log.action && log.action.includes('busy_mode_config_updated')) {
      const admin = parsed.adminUserId ? `Người quản trị #${parsed.adminUserId}` : 'Người quản trị';
      const cfg = parsed.newConfig || {};
      return (
        <div className="text-sm space-y-1">
          <div>{admin} đã cập nhật cấu hình chế độ bận:</div>
          <div className="ml-2">
            {cfg.maxOrdersPerWindow !== undefined && <div>- Số đơn tối đa: <span className="font-mono">{cfg.maxOrdersPerWindow}</span></div>}
            {cfg.timeWindowMinutes !== undefined && <div>- Khoảng thời gian (phút): <span className="font-mono">{cfg.timeWindowMinutes}</span></div>}
          </div>
          <button
            className="text-xs text-blue-600 underline mt-1"
            onClick={() => setDetailsModal({ open: true, title: 'Chi tiết cấu hình', contentRaw: JSON.stringify(parsed, null, 2), summary: renderLogDetails(log), showRaw: false })}
          >
    ...
    if (log.action && log.action.includes('busy_mode_config_updated')) {
      const admin = parsed.adminUserId ? `Người quản trị #${parsed.adminUserId}` : 'Người quản trị';
      const cfg = parsed.newConfig || parsed.newConfig || {};
      return (
        <div className="text-sm space-y-1">
          <div>{admin} đã cập nhật cấu hình chế độ bận:</div>
          <div className="ml-2">
    ...
    // default: pretty print keys
    return (
      <div className="text-sm space-y-1">
        {Object.keys(parsed).slice(0,4).map((k) => (
          <div key={k}><span className="text-gray-600">{k}:</span> <span className="font-mono">{String(parsed[k]).substring(0,80)}{String(parsed[k]).length>80?'...':''}</span></div>
        ))}
      </div>
    );
  };

  const handleToggleBusyMode = async (enabled) => {
    try {
      setSavingBusyMode(true);
      const res = await api.post(`/anti-spam/busy-mode/${store?.id}`, { enabled });

      if (res.data.success) {
        toast.success(`Chế độ bận đã ${enabled ? 'bật' : 'tắt'}`);
        // Refresh busy mode status
        const busyModeRes = await api.get(`/anti-spam/busy-mode-status/${store?.id}`);
        if (busyModeRes.data.success) {
          setBusyModeStatus(busyModeRes.data.data);
        }
      }
      ...
      toast.error('Không thể thay đổi chế độ bận');
    } finally {
      setSavingBusyMode(false);
    }
  };

  const handleUpdateBusyModeConfig = async () => {
    try {
      setSavingBusyMode(true);
      const res = await api.put(`/anti-spam/busy-mode-config/${store?.id}`, busyModeConfig);

      if (res.data.success) {
        toast.success('Cấu hình chế độ bận đã được cập nhật');
        // Refresh busy mode status
        const busyModeRes = await api.get(`/anti-spam/busy-mode-status/${store?.id}`);
        if (busyModeRes.data.success) {
          setBusyModeStatus(busyModeRes.data.data);
      ...
      console.error('Update busy mode config error:', error);
  ...
  return (
    <Layout>
      ...
      <div className="w-full">
        <div className="relative overflow-x-auto px-3 md:px-6">
          ...
          <button
            onClick={() => setActiveTab('anti-spam')}
            className={`px-3 md:px-4 py-2 text-xs sm:text-sm md:text-base font-semibold transition flex items-center gap-2 whitespace-nowrap rounded-lg ${
              activeTab === 'anti-spam'
                ? 'bg-purple-100 text-purple-700 shadow-inner'
                : 'text-gray-800 hover:bg-gray-100'
        ...
        {activeTab === 'orders' && (
          <div className="card">
            ...
            {orders.filter(order => {
              ...
              <p className="text-gray-600">Chưa có đơn hàng nào hôm nay</p>
            ) : (
              ...
              <div className="bg-white p-4 sm:p-6 rounded-xl border border-gray-200 shadow-sm">
                ...
                <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm text-blue-800 font-medium mb-1">Lưu ý về trạng thái quán</p>
                      <ul className="text-xs text-blue-700 space-y-1">
                        <li>• Khi quán đóng cửa, khách hàng sẽ không thể đặt hàng</li>
                        <li>• Bạn có thể thay đổi trạng thái bất cứ lúc nào</li>
                      </ul>
                    </div>
                  </div>
                </div>
      ...
      {activeTab === 'anti-spam' && (
        <div className="card px-3 sm:px-4 md:px-6">
          <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">Chống Spam & Bảo Mật</h2>

          {/* Refresh Button */}
          <div className="mb-6">
            <button
              onClick={fetchAntiSpamData}
              disabled={loadingAntiSpam}
              className="btn btn-primary flex items-center gap-2"
            >
              <RefreshIcon className="w-4 h-4" />
              {loadingAntiSpam ? 'Đang tải...' : 'Làm mới dữ liệu'}
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <DashboardCard>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-red-600">{antiSpamStats.activeIPBlocks || 0}</p>
                  </div>
                </div>
              </DashboardCard>
              ...
              <DashboardCard>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-600">Thiết bị bị chặn</p>
                    <p className="text-xl font-bold text-orange-600">{antiSpamStats.activeDeviceBlocks || 0}</p>
                  </div>
                </div>
              </DashboardCard>
              ...
              <DashboardCard>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-600">Spam logs (24h)</p>
                    <p className="text-xl font-bold text-blue-600">{antiSpamStats.spamLogsLast24h || 0}</p>
                  </div>
                </div>
              </DashboardCard>
              ...
              <DashboardCard>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-600">Spam logs (1h)</p>
                    <p className="text-xl font-bold text-purple-600">{antiSpamStats.spamLogsLastHour || 0}</p>
                  </div>
                </div>
              </DashboardCard>
            </div>
          ...
          {/* Busy Mode Section */}
          <div className="mb-6">
            <h3 className="text-lg font-bold mb-4">Chế độ "Quán đang bận"</h3>
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h4 className="font-semibold text-gray-900 mb-1">Chế độ bận thủ công</h4>
                  <p className="text-sm text-gray-600">
                    Tắt tạm thời để quán nghỉ ngơi hoặc bảo trì
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    className="sr-only peer"
                    checked={busyModeStatus?.manualBusyMode || false}
                    onChange={(e) => handleToggleBusyMode(e.target.checked)}
                  ...
                  <span className="ml-3 text-sm font-medium text-gray-900">
                    {busyModeStatus?.manualBusyMode ? 'BẬN' : 'BÌNH THƯỜNG'}
                  </span>
                </label>
              </div>

              <div className="border-t border-gray-200 pt-4">
                <h4 className="font-semibold text-gray-900 mb-3">Cấu hình tự động</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      Số đơn tối đa trong khoảng thời gian
                    </label>
                    <input
                      type="number"
                      min="1"
                      max="1000"
                      value={busyModeConfig.maxOrdersPerWindow}
                      onChange={(e) => setBusyModeConfig(prev => ({
                        ...prev,
                        maxOrdersPerWindow: parseInt(e.target.value) || 20
                      }))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="1"
                      max="1440"
                      value={busyModeConfig.timeWindowMinutes}
                      onChange={(e) => setBusyModeConfig(prev => ({
                        ...prev,
                ...
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    Trạng thái hiện tại: <span className={`font-semibold ${
                      busyModeStatus?.isBusy ? 'text-red-600' : 'text-green-600'
                    }`}>
                      {busyModeStatus?.isBusy ? 'ĐANG BẬN' : 'BÌNH THƯỜNG'}
                    </span>
                    {busyModeStatus?.isBusy && (
                      <span className="ml-2">
                        ({busyModeStatus.manualBusyMode ? 'thủ công' : 'tự động'})
                      </span>
                    )}
                  </div>
                  <button
                    onClick={handleUpdateBusyModeConfig}
                    disabled={savingBusyMode}
                    className="btn btn-primary"
                  >
                    {savingBusyMode ? 'Đang lưu...' : 'Lưu cấu hình'}
                  </button>
                </div>
              </div>
            </div>
          ...
          {/* Blocked IPs Section */}
          <div className="mb-6">
            <h3 className="text-lg font-bold mb-4">IP bị chặn ({blockedIPs.length})</h3>
                      ...
                      {blockedIPs.map((ip) => (
                        <tr key={ip.id}>
                          ...
                          <td className="px-4 py-3 text-sm">
                            {new Date(ip.createdAt).toLocaleString('vi-VN')}
                          </td>
                          <td className="px-4 py-3 text-sm">
                            {ip.blockedUntil
                              ? new Date(ip.blockedUntil).toLocaleString('vi-VN')
                              : 'Vĩnh viễn'
                            }
                          </td>
                        </tr>
          ...
          <div className="mb-6">
            <h3 className="text-lg font-bold mb-4">Thiết bị bị chặn ({blockedDevices.length})</h3>
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {blockedDevices.map((device) => (
                        <tr key={device.id}>
                          ...
                          <td className="px-4 py-3 text-sm">{device.reason}</td>
                          <td className="px-4 py-3 text-sm">
          ...
          {/* Recent Spam Logs */}
          <div>
            <h3 className="text-lg font-bold mb-4">Spam logs gần đây</h3>
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              {spamLogs.length === 0 ? (
                <div className="p-6 text-center text-gray-500">
                  Chưa có hoạt động spam nào
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {spamLogs.map((log) => (
                        <tr key={log.id}>
                          ...
                          <td className="px-4 py-3 text-sm font-mono text-xs flex items-center gap-2">
                            <span title={log.deviceId || '-'} className="truncate max-w-[180px] block">
                              {log.deviceId ? `${log.deviceId.substring(0, 12)}...` : '-'}
                            </span>
                            {log.deviceId && (
                              (blockedDevices && blockedDevices.some(b => b.deviceId === log.deviceId)) || (log.action && log.action.includes('block')) ? (
                                <button
                                  onClick={() => handleUnblockDevice(log.deviceId)}
                                  className="text-xs px-2 py-1 bg-green-50 text-green-700 rounded-md hover:bg-green-100"
                                  title="Huỷ chặn thiết bị này"
                                >
                                  Huỷ chặn
                                </button>
                              ) : (
                                <button
                                  ...
                                  className="text-xs px-2 py-1 bg-red-50 text-red-700 rounded-md hover:bg-red-100"
                          ...
                          <td className="px-4 py-3 text-sm">
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              log.action.includes('block') ? 'bg-red-100 text-red-800' :
                              log.action.includes('spam') ? 'bg-orange-100 text-orange-800' :
                              'bg-blue-100 text-blue-800'
                            }`}>
                              {log.action === 'device_spam_attempt' ? 'Cố spam thiết bị' :
                               log.action === 'device_blocked_attempt' ? 'Thiết bị bị chặn' :
                               log.action}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-sm">
    ...
    {detailsModal.open && (
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        <div className="absolute inset-0 bg-black/60" onClick={() => setDetailsModal({ open: false, title: '', contentRaw: '', summary: null, showRaw: false })} />
        <div className="relative z-10 max-w-3xl w-full mx-4 bg-white rounded-xl shadow-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold">{detailsModal.title}</h3>
            <button onClick={() => setDetailsModal({ open: false, title: '', contentRaw: '', summary: null, showRaw: false })} className="text-sm text-gray-500">Đóng</button>
          </div>

          {/* Summary (human-friendly only) */}
          <div className="mb-4">
            {detailsModal.summary ? detailsModal.summary : <div className="text-sm text-gray-600">Không có thông tin tóm tắt.</div>}
          </div>
        </div>
      </div>
    )}
